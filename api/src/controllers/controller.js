...
const postDog = async (
  ...
  try {
    ...
    const dogName = dogsApiDB.find(
      (dog) => dog.name.toLowerCase() === nameLowerCase.trim()
    );
    if (dogName) {
      throw new Error(
        `Dog ${name} already exists in the API or in the Database`
      );
    } else if (
      !name ||
      !minHeight ||
      ...
      throw new Error(
        "The height, weight ow life span value cannot be negative"
      );
