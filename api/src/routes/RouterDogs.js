...
router.post("/", async (req, res) => {
  try {
    ...
    // Separar la cadena de temperamentos en un arreglo
    const temperamentsArray = temperament.split(",").map((item) => item.trim());

    if (temperamentsArray.length === 0 || temperamentsArray[0] === "") {
    ...
    res
      .status(200)
      .json({ message: `Created dog ${newDog.name} with id ${newDog.id}` });
  } catch (error) {
    res.status(400).json({ error: error.message });
  }
});
